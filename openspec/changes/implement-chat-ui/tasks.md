...
**依赖**: 无  
**交付物**:
- [ ] 创建 `src/app/(main)/chat/[topicId]/page.tsx`
- [ ] 实现基础页面布局（Header + Content + Input 区域）
- [ ] 添加路由参数验证
- [ ] 配置页面 metadata
...
**依赖**: 无  
**交付物**:
- [ ] 创建 `src/app/api/topics/[id]/route.ts`
- [ ] 实现 GET handler 获取 Topic 和关联的 Messages
- [ ] 添加错误处理（Topic 不存在、数据库错误）
- [ ] 编写单元测试
...
- [ ] 定义默认 query 选项（staleTime, cacheTime）
- [ ] 添加 devtools（开发环境）
...
- [ ] 集成 Ant Design X Bubble 组件
- [ ] 实现用户/AI 消息样式差异
- [ ] 添加 Markdown 渲染支持（react-markdown）
- [ ] 实现代码块语法高亮（prism-react-renderer）
- [ ] 添加复制按钮到代码块
- [ ] 编写组件测试
...
**依赖**: Task 1.2  
**交付物**:
- [ ] 创建 `src/lib/chat/message-tree.ts`
- [ ] 实现 `buildMessageChain(messages, leafId)` 函数
- [ ] 实现 `findSiblings(messages, messageId)` 函数
- [ ] 实现 `getDefaultLeaf(messages)` 函数
- [ ] 编写单元测试（覆盖各种树结构）
...
**依赖**: Task 1.3  
**交付物**:
- [ ] 创建 `src/components/chat/MessageInput.tsx`
- [ ] 集成 Ant Design X Sender 组件
- [ ] 实现多行输入和自动高度调整
- [ ] 添加 Ctrl+Enter 发送快捷键
- [ ] 实现输入验证（非空检查）
- [ ] 添加发送状态指示
...
- [ ] 定义状态接口（currentBranchPath, inputDraft, isComposing）
- [ ] 实现 actions（setCurrentBranch, updateDraft）
- [ ] 添加 persist 中间件（保存草稿）
- [ ] 编写单元测试
...
**交付物**:
- [ ] 创建 `src/components/chat/BranchNavigator.tsx`
- [ ] 实现分支选择下拉菜单
- [ ] 显示每个分支的预览文本
- [ ] 高亮当前分支
- [ ] 集成 chat-store 更新分支路径
- [ ] 编写组件测试
...
- [ ] 使用 TanStack Query 获取 Topic 数据
- [ ] 连接 MessageList 和 MessageInput
- [ ] 实现消息发送处理
- [ ] 添加加载和错误状态
- [ ] 编写集成测试
...
**交付物**:
- [ ] 在 ChatContainer 中使用 `useChat` hook
- [ ] 配置 API 端点和请求参数
- [ ] 实现乐观更新
- [ ] 处理流式响应状态（loading, streaming, error）
- [ ] 实现响应完成后的数据刷新
- [ ] 编写集成测试
