...
#### Scenario: 系统主题优先
- **GIVEN** 用户将主题模式设置为 `system`
- **WHEN** 操作系统主题在浅色与深色之间切换
- **THEN** 应用 **SHALL** 自动跟随系统主题更新
- **AND** 页面模板与组件 **SHALL** 使用同一套语义设计标记完成切换
...
- **GIVEN** 用户进入设置相关页面
- **WHEN** 页面完成渲染
- **THEN** 页面 **SHALL** 使用与其他主页面一致的全局壳层
- **AND** 设置导航 **SHALL** 作为模块级二级导航呈现
- **AND** 页面 **SHALL NOT** 出现与全局导航割裂的独立壳层

### Requirement: 页面模板统一

系统 **MUST** 统一核心页面模板契约，优先保证信息架构与布局节奏一致。

#### Scenario: 核心页面模板接入
- **GIVEN** 用户访问首页、会话列表、聊天页或设置页
- **WHEN** 页面渲染
- **THEN** 页面 **SHALL** 复用统一模板结构（AppShell、TopBar、PageFrame）
- **AND** 各页面 **SHALL** 保持一致的内容宽度、间距节奏与标题层级

#### Scenario: 模块级二级导航插槽
- **GIVEN** 某模块存在内部子导航需求（如设置）
- **WHEN** 页面模板渲染
- **THEN** 系统 **SHALL** 通过统一模板中的二级导航插槽承载模块导航
- **AND** 不同模块 **SHALL** 遵循一致的插槽位置与交互样式
